import { getAdminChatIds, saveBooking } from './db.js';
import { formatBookingMessage, notifyRecipients } from './telegram.js';
...
async function resolveRecipientIds(env) {
  if (env.DB) {
    try {
      const adminIds = await getAdminChatIds(env.DB);
      if (adminIds.length > 0) return adminIds;
    } catch (err) {
      console.error('D1 admin lookup failed', err);
    }
  }
...
export async function handleBooking(request, env) {
  ...
  if (env.DB) {
    try {
      await saveBooking(env.DB, body);
    } catch (err) {
      console.error('D1 booking save failed', err);
    }
  }
